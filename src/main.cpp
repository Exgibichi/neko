...
CAmount GetProofOfStakeReward(int64_t nCoinAge)
{
    ...
    int64_t nSubsidy = nCoinAge * 33 / (1 * 1 + 8) * nRewardCoinYear;
...
bool ppcoinContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
  ...
  // compute nStakeModifierChecksum end
    ...
    if (!CheckStakeModifierCheckpoints(pindex->nHeight, nStakeModifierChecksum))
        return error("ConnectBlock() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016llx, nStakeModifierChecksum=%08x", pindex->nHeight, nStakeModifier, nStakeModifierChecksum);
